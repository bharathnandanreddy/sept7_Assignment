...
const Table= (props)=>{
    ...
    let rows=Object.keys(props.books).map(key=>{
        return (
            ...
            <Row  delClicked={props.delClicked} clicked={props.clicked} key={key} checks={props.checks} book={props.books[key]}/>
        )
    })
    ...
    return(
        ...
        <table className="table table-bordered table-hover ">
            <thead>
                <tr>
                {props.checks['author'].checked?   <th style={{width: "20%"}}  onClick={() => props.sortData("author")}>Author <img alt="img" className={classes.Sort} width="18px" src={sort} /></th>: null}
                ...
                {props.checks['author'].checked || props.checks['title'].checked || props.checks['publisher'].checked || props.checks['group'].checked? <th style={{width: "10%"}}></th>: null}
                </tr>
                </thead>
                <tbody>
                    {rows}
